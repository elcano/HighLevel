...
//6) 47.760681, -122.190255
  ...
  #define currentlocation    -1      //currentLocation
   ...
   //methods
   void initialize_Map(Position&vehiclePosition);
   void SelectMap(Position& MapOrgin, Position& vehiclePosition, const char *fileName, char *nearestMap);
   bool LoadMap(char *fileName);
   void FindDistances(Junction *Map);
   void GetGoals(Position&ori, Junction *mapNodes);
   ...
   int PlanPath(Waypoint&start, Waypoint&destination);  //calls find
   int FindPath(Waypoint&start, Waypoint&destination);  //calls build
   int BuildPath(long&j, Waypoint&start, Waypoint&destination);



public:
   Planner(Position&MapOrgin);
   ~Planner()
   {
   }

   //elcano::Position *origin;
