 ...
 * constructor
 *****************************************************************************************************/
Pilot::Pilot(Position vehiclePosition)
{
#ifdef DEBUG
   Serial.println("Starting Planning");
#endif
   myPlanner = new Planner(vehiclePosition);

   //Trike state starts Straight
   turn_direction = 0; // going straight
#ifdef DEBUG
   Serial.println("Entered Pilot setup");
#endif
   state = STARTING;
...
int Pilot::get_turn_direction_angle(int n, Waypoint&estPos)
{
   if (state == ENTER_TURN)
   {
      ...
      // The whole reason for (Evector_x1000, Nvector_x1000) is to be able to do the arithmetic below
      // using long int instead of double.   TCF 8/11/20
      ...
      turn_direction_angle = (int)(turn_dir * 180 / PIf); //angle is degrees
      if (cross > 0)
      {
...
bool Pilot::proper_heading(Waypoint&estimated_pos, int n)
{
   ...
   double properHeading = atan2(northDiff, eastDiff) / TO_RADIANS;
...
void Pilot::Pilot_communicate_LowLevel()
{
   ...
   delay(1000);    //usually only needed if using a Serial Print
#endif
#endif
...
void Pilot::update(Waypoint&estimated_pos, Waypoint&old_pos)
{
   if (first)
   {
      initializePosition(estimated_pos, old_pos);
   }
   //Determining the state of the Trike
   find_state(TURN_RADIUS_MM, next, estimated_pos);
#ifdef DEBUG3
   Serial.println("State is: " + String(wrdState[state]) + ", path # " + String(next));
#endif
...
#ifdef DEBUG3
   Serial.println("setting turn angle to: " + String(turn_direction));
#endif
