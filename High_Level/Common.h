// Common.h - Defines objects for Waypoints and Junctions.
#ifndef _COMMON_H_
#define _COMMON_H_
#pragma once
...
{
public:
   double latitude; //  = ORIGIN_LAT;
   double longitude; // = ORIGIN_LONG;
   float cos_lat;
   void operator=(Position& right);

   //cos_lat will be initialized depending on latDeg and latMin
   Position();
   Position(double lat, double lon);  //constructor
...
public:
   Waypoint();
   Waypoint(Position Lat_Lon);   // Constructor
   // Since the Arduino Mega supports only 6 digits of precision in float/double,[Due supports 14]
   // all latitudes and longitudes are recorded as integers.
   // The (east_mm and north_mm) position carries more precision.
   // They have 32 bits; double on Mega is 32 bits total, with only 24 for mantissa.
   double latitude;
   double longitude;

   long east_mm;          // x is east; max is 2147 km
   long north_mm;         // y is true north
   long sigma_mm = DEFAULT_GPS_ERROR_MM;  // standard deviation of position for GPS.
   ...
   void Compute_mm(Position&origin);
   void Compute_LatLon(Position&origin);
   bool AcquireGPRMC(unsigned long max_wait_ms);
   bool AcquireGPGGA(unsigned long max_wait_ms);
   void fuse(Waypoint reading, int deltaT_ms, Position&origin);
   ...
   void CreateFrom(elcano::Position right);
};
// end Waypoint
